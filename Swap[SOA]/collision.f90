!===========================================================================
      ...
      real, dimension(lx,ly,0:npop-1)     :: tmpzmR,tmpzpR,tmpzpS,tmpzmS
      real, dimension(lx,0:lz+1,0:npop-1) :: tmpypS,tmpypR,tmpymS,tmpymR
      real tempDist
      integer ix, iy, iz, ip
      integer status_array(MPI_STATUS_SIZE,4), req(4), ilen

      tmpzmS(:,:,:) = f(1:lx,1:ly,1,:)
      tmpzpS(:,:,:) = f(1:lx,1:ly,lz,:)
      ...
      f(1:lx,0,0:lz+1,:) = tmpymR(:,:,:)
      f(1:lx,ly+1,0:lz+1,:) = tmpypR(:,:,:)
      f(1:lx,1:ly,0,:) = tmpzmR(:,:,:)
      f(1:lx,1:ly,lz+1,:) = tmpzpR(:,:,:)

      do iz = 1,lz
      do iy = 1,ly
       if(iy==1)then
          f(1,iy,iz,4) = f(1,iy-1,iz,3)
          f(lx,iy,iz,4) = f(lx,iy-1,iz,3)
          f(lx,iy,iz,10) = f(lx-1,iy-1,iz,7)
          f(1,iy,iz,9) = f(2,iy-1,iz,8)
          f(1,iy,iz,18) = f(1,iy-1,iz-1,15)
          f(lx,iy,iz,18) = f(lx,iy-1,iz-1,15)
        endif

        if(iz==1)then
          f(1,iy,iz,6) = f(1,iy,iz-1,5)
          f(lx,iy,iz,6) = f(lx,iy,iz-1,5)
          f(1,iy,iz,13) = f(2,iy,iz-1,12)
          f(lx,iy,iz,14) = f(lx-1,iy,iz-1,11)
          f(1,iy,iz,17) = f(1,iy+1,iz-1,16)
          f(lx,iy,iz,17) = f(lx,iy+1,iz-1,16)
          if(iy .ne. 1)then
            f(18,1,iy,iz) = f(15,1,iy-1,iz-1)
            f(18,lx,iy,iz) = f(15,lx,iy-1,iz-1)
          endif
        endif
        ...
        tempDist = f(1,iy,iz,1)
        f(1,iy,iz,1) = f(2,iy,iz,2)
        f(2,iy,iz,2) = tempDist

        tempDist = f(1,iy,iz,3)
        f(1,iy,iz,3) = f(1,iy+1,iz,4)
        f(1,iy+1,iz,4) = tempDist

        tempDist = f(lx,iy,iz,3)
        f(lx,iy,iz,3) = f(lx,iy+1,iz,4)
        f(lx,iy+1,iz,4) = tempDist

        tempDist = f(1,iy,iz,5)
        f(1,iy,iz,5) = f(1,iy,iz+1,6)
        f(1,iy,iz+1,6) = tempDist

        tempDist = f(lx,iy,iz,5)
        f(lx,iy,iz,5) = f(lx,iy,iz+1,6)
        f(lx,iy,iz+1,6) = tempDist

        tempDist = f(1,iy,iz,7)
        f(1,iy,iz,7) = f(2,iy+1,iz,10)
        f(2,iy+1,iz,10) = tempDist

        tempDist = f(lx,iy,iz,8)
        f(lx,iy,iz,8) = f(lx-1,iy+1,iz,9)
        f(lx-1,iy+1,iz,9) = tempDist

        tempDist = f(1,iy,iz,11)
        f(1,iy,iz,11) = f(2,iy,iz+1,14)
        f(2,iy,iz+1,14) = tempDist

        tempDist = f(lx,iy,iz,12)
        f(lx,iy,iz,12) = f(lx-1,iy,iz+1,13)
        f(lx-1,iy,iz+1,13) = tempDist

        tempDist = f(1,iy,iz,15)
        f(1,iy,iz,15) = f(1,iy+1,iz+1,18)
        f(1,iy+1,iz+1,18) = tempDist

        tempDist = f(lx,iy,iz,15)
        f(lx,iy,iz,15) = f(lx,iy+1,iz+1,18)
        f(lx,iy+1,iz+1,18) = tempDist

        tempDist = f(1,iy,iz,16)
        f(1,iy,iz,16) = f(1,iy-1,iz+1,17)
        f(1,iy-1,iz+1,17) = tempDist

        tempDist = f(lx,iy,iz,16)
        f(lx,iy,iz,16) = f(lx,iy-1,iz+1,17)
        f(lx,iy-1,iz+1,17) = tempDist
      enddo
      enddo
      ...
      do ix = 2,lx-1
        if(iy==1)then
          f(ix,iy,iz,4) = f(ix,iy-1,iz,3)
          f(ix,iy,iz,10) = f(ix-1,iy-1,iz,7)
          f(ix,iy,iz,9) = f(ix+1,iy-1,iz,8)
          f(ix,iy,iz,18) = f(ix,iy-1,iz-1,15)
        endif

        if(iz==1)then
          f(ix,iy,iz,6) = f(ix,iy,iz-1,5)
          f(ix,iy,iz,13) = f(ix+1,iy,iz-1,12)
          f(ix,iy,iz,14) = f(ix-1,iy,iz-1,11)
          f(ix,iy,iz,17) = f(ix,iy+1,iz-1,16)
          if(iy .ne. 1) f(ix,iy,iz,18) = f(ix,iy-1,iz-1,15)
        endif
        ...
        tempDist = f(ix,iy,iz,1)
        f(ix,iy,iz,1) = f(ix+1,iy,iz,2)
        f(ix+1,iy,iz,2) = tempDist

        tempDist = f(ix,iy,iz,3)
        f(ix,iy,iz,3) = f(ix,iy+1,iz,4)
        f(ix,iy+1,iz,4) = tempDist

        tempDist = f(ix,iy,iz,5)
        f(ix,iy,iz,5) = f(ix,iy,iz+1,6)
        f(ix,iy,iz+1,6) = tempDist

        tempDist = f(ix,iy,iz,7)
        f(ix,iy,iz,7) = f(ix+1,iy+1,iz,10)
        f(ix+1,iy+1,iz,10) = tempDist

        tempDist = f(ix,iy,iz,8)
        f(ix,iy,iz,8) = f(ix-1,iy+1,iz,9)
        f(ix-1,iy+1,iz,9) = tempDist

        tempDist = f(ix,iy,iz,11)
        f(ix,iy,iz,11) = f(ix+1,iy,iz+1,14)
        f(ix+1,iy,iz+1,14) = tempDist

        tempDist = f(ix,iy,iz,12)
        f(ix,iy,iz,12) = f(ix-1,iy,iz+1,13)
        f(ix-1,iy,iz+1,13) = tempDist

        tempDist = f(ix,iy,iz,15)
        f(ix,iy,iz,15) = f(ix,iy+1,iz+1,18)
        f(ix,iy+1,iz+1,18) = tempDist
        
        tempDist = f(ix,iy,iz,16)
        f(ix,iy,iz,16) = f(ix,iy-1,iz+1,17)
        f(ix,iy-1,iz+1,17) = tempDist

        do ip =0, npop-1
          f9(ipopp(ip)) = f(ix,iy,iz,ip)
        enddo
        
        call collision_MRT(f9,ix,iy,iz,f(ix,iy,iz,:))
      enddo
      enddo
      enddo
      ...
      do iy = 1,ly
        do ip =0, npop-1
          f9(ipopp(ip)) = f(lx,iy,iz,ip)
        enddo
        ix = lx
        call collision_MRT(f9,ix,iy,iz,f(ix,iy,iz,:))

        do ip =0, npop-1
          f9(ipopp(ip)) = f(1,iy,iz,ip)
        enddo
        ix = 1
        call collision_MRT(f9,ix,iy,iz,f(ix,iy,iz,:))
      enddo
...
! velocity space into moment space.
      ...
      subroutine collision_MRT(f9,ix,iy,iz, f9r)
      use var_inc
      implicit none 

      real, dimension(0:npop-1) :: f9, f9r
