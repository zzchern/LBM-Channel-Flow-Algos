        ...
        do ip = 0,npop-1
          ...
          if(sw .eq. 1)then
            if(imove < 1 .or. imove > lx)then
              f9(ip) = f(ipopp(ip),ix,iy,iz) + Fbar(ip)/2.
            elseif(jmove < 1)then
              f9(ip) = tmpymR(ip,imove,kmove) + Fbar(ip)/2.
            elseif(jmove > ly)then
              f9(ip) = tmpypR(ip,imove,kmove) + Fbar(ip)/2.
            elseif(kmove < 1)then
              f9(ip) = tmpzmR(ip,imove,jmove) + Fbar(ip)/2.
            elseif(kmove > lz)then
              f9(ip) = tmpzpR(ip,imove,jmove) + Fbar(ip)/2.
            else
              f9(ip) = f(ip,imove,jmove,kmove) + Fbar(ip)/2.
            endif
          else
            if(imove < 2 .or. imove > lx + 1)then
              f9(ip) = f(ipopp(ip),ix,iy,iz) + Fbar(ip)/2.
            elseif(jmove < 2)then
              f9(ip) = tmpymR(ip,imove-1,kmove-1) + Fbar(ip)/2.
            elseif(jmove > ly + 1)then
              f9(ip) = tmpypR(ip,imove-1,kmove-1) + Fbar(ip)/2.
            elseif(kmove < 2)then
              f9(ip) = tmpzmR(ip,imove-1,jmove-1) + Fbar(ip)/2.
            elseif(kmove > lz + 1)then
              f9(ip) = tmpzpR(ip,imove-1,jmove-1) + Fbar(ip)/2.
            else
              f9(ip) = f(ip,imove,jmove,kmove) + Fbar(ip)/2.
            endif
          endif
...
!        +f(15,:,:,:)+f(16,:,:,:)+f(17,:,:,:)+f(18,:,:,:)
      ...
      if(sw .eq. 1)then
        ...
        rho = f(0,2:lx+1,2:ly+1,2:lz+1)
        do ip = 1,npop-1
          rho = rho + f(ip,2:lx+1,2:ly+1,2:lz+1)
        end do
      endif
