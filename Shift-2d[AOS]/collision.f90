...
! velocity space into moment space.
      ...
      if(sw .eq. 1)then
        ...
        sp = -1
        tmpzmS(:,:,:) = f(:,:,2:ly+1,2)
        tmpzpS(:,:,:) = f(:,:,2:ly+1,lz+1)
        tmpymS(:,:,1:lz) = f(:,:,2,2:lz+1)
        tmpypS(:,:,1:lz) = f(:,:,ly+1,2:lz+1)
        sx = 1; sy = 2; sz = 2; step = 1
        ex = lx; ey = ly + 1; ez = lz + 1
      endif
      ...
      do ix = sx,ex,step
        if(sw .eq. 1)then
          ...
          uy9s = uy9*uy9
          uz9s = uz9*uz9
          fx9 = force_realx(ix,iy-1,iz-1)
          fy9 = force_realy(ix,iy-1,iz-1)
          fz9 = force_realz(ix,iy-1,iz-1)
        endif
