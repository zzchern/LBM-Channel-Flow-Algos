...
! velocity space into moment space.
      ...
      integer ip, ilen, sx, sy, sz, ex, ey, ez, step
      ...
      if(sw .eq. 1)then
        ...
        sx = lx; sy = ly; sz = lz; step = -1
        ex = 1; ey = 1; ez = 1
      else
        sp = -1
        tmpzmS(:,:,:) = f(2:lx+1,2:ly+1,2,:)
        tmpzpS(:,:,:) = f(2:lx+1,2:ly+1,lz+1,:)
        tmpymS(:,1:lz,:) = f(2:lx+1,2,2:lz+1,:)
        tmpypS(:,1:lz,:) = f(2:lx+1,ly+1,2:lz+1,:)
        sx = 2; sy = 2; sz = 2; step = 1
        ex = lx + 1; ey = ly + 1; ez = lz + 1
      endif
      ...
      do ix = sx,ex,step
        if(sw .eq. 1)then
          ...
          rho9 = rho(ix-1,iy-1,iz-1)
          ux9 = ux(ix-1,iy-1,iz-1)
          uy9 = uy(ix-1,iy-1,iz-1)
          uz9 = uz(ix-1,iy-1,iz-1)
          ux9s = ux9*ux9
          uy9s = uy9*uy9
          uz9s = uz9*uz9
          fx9 = force_realx(ix-1,iy-1,iz-1)
          fy9 = force_realy(ix-1,iy-1,iz-1)
          fz9 = force_realz(ix-1,iy-1,iz-1)
        endif
        ...
        do ip = 0,npop-1
          ...
          if(sw .eq. 1)then
            if(imove < 1 .or. imove > lx)then
              f9(ip) = f(ix,iy,iz,ipopp(ip)) + Fbar(ip)/2.
            elseif(jmove < 1)then
              f9(ip) = tmpymR(imove,kmove,ip) + Fbar(ip)/2.
            elseif(jmove > ly)then
              f9(ip) = tmpypR(imove,kmove,ip) + Fbar(ip)/2.
            elseif(kmove < 1)then
              f9(ip) = tmpzmR(imove,jmove,ip) + Fbar(ip)/2.
            elseif(kmove > lz)then
              f9(ip) = tmpzpR(imove,jmove,ip) + Fbar(ip)/2.
            else
              f9(ip) = f(imove,jmove,kmove,ip) + Fbar(ip)/2.
            endif
          else
            if(imove < 2 .or. imove > lx + 1)then
              f9(ip) = f(ix,iy,iz,ipopp(ip)) + Fbar(ip)/2.
            elseif(jmove < 2)then
              f9(ip) = tmpymR(imove-1,kmove-1,ip) + Fbar(ip)/2.
            elseif(jmove > ly + 1)then
              f9(ip) = tmpypR(imove-1,kmove-1,ip) + Fbar(ip)/2.
            elseif(kmove < 2)then
              f9(ip) = tmpzmR(imove-1,jmove-1,ip) + Fbar(ip)/2.
            elseif(kmove > lz + 1)then
              f9(ip) = tmpzpR(imove-1,jmove-1,ip) + Fbar(ip)/2.
            else
              f9(ip) = f(imove,jmove,kmove,ip) + Fbar(ip)/2.
            endif
          endif
        ...
        f(ix+sp,iy+sp,iz+sp,:) = f9(:) + 0.5*Fbar(:)
